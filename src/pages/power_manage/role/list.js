...
import {list_fn,list_detail_fn} from "actions/power_manage/role/list";
  ...
  render() {
      const columns = [
        ...
        },
        {
          ...
          key: 'operation',
          render: (text, record, index) =>{
              return <div className={'l_power_list_operation'+' l_role_list_operation'+record.id}>
                ...
                <Popconfirm_new position={'l_role_list_operation'+record.id} placement="topLeft" title={'确认要删除吗？'} onConfirm={()=>this.delete(record.id)} okText="删除" cancelText="取消">
                     <span>删除</span>
                </Popconfirm_new>
...
export default connect((state) => ({list:state.role_list}), {edit_modal_fn,list_fn,list_detail_fn})(Power);
