...
import {list_fn,list_detail_fn} from "actions/power_manage/user/list";
   ...
   .then(function(response) {
        that.setState({
         ...
         if(response.code=='0'){
            ...
            let params=that.props.list.params;
            that.search_fn(params)
         }
  ...
  render() {
      const columns = [
        ...
        },
        {
          ...
          key: 'operation',
          render: (text, record, index) =>{
              return <div className={'l_power_list_operation'+' l_user_list_operation'+record.id}>
                ...
                <Popconfirm_new position={'l_user_list_operation'+record.id} placement="topLeft" title={'确认要删除吗？'} onConfirm={()=>this.delete(record.id)} okText="删除" cancelText="取消">
                     <span>删除</span>
                </Popconfirm_new>
    ...
    return (
        <div className='l_power'>
            <Common_search search={this.search_fn} types={['name_phone_email','status']}/>
...
export default connect((state) => ({list:state.user_list}), {edit_modal_fn,list_fn,list_detail_fn})(Power);
